...
function groupSessionsByTime(sessions: V2Session[]): [string, V2Session[]][] {
  const now = new Date();
  ...
  const result: [string, V2Session[]][] = [
    ...
    ["最近7天", groups["最近7天"]],
    ["更早", groups["更早"]],
  ].filter(([, items]) => items.length > 0) as [string, V2Session[]][];
  return result;
}
        ...
        {grouped.map(([label, sessions]) => (
          <div key={label}>
            {!isCollapsed && sessions.length > 0 && (
              <div className="px-2 py-1 text-xs font-medium" style={{ color: P2 }}>
                {label}
              </div>
            )}
            {sessions.map((s) => {

...
export async function editImage(params: { prompt: string; images: File[]; model?: string; n?: number; size?: string }): Promise<GenerateImageResponse> {
  const fd = new FormData(); fd.append("model", params.model || "gpt-image-2"); fd.append("prompt", params.prompt); fd.append("n", String(params.n || 1));
...
export async function submitGenerationTask(params: Record<string, unknown>): Promise<ImageTaskItem> { const res = await fetch(`${MANAGEMENT_BASE}/api/image-tasks/generations`, { method: "POST", headers: authHeaders(), body: JSON.stringify({ client_task_id: params.client_task_id, prompt: params.prompt, model: params.model || "gpt-image-2", size: params.size }) }); if (!res.ok) { const err = await res.json().catch(() => ({ error: res.statusText })); throw new Error(stringifyErrorPayload(err) || `提交任务失败: ${res.status}`); } return res.json(); }
